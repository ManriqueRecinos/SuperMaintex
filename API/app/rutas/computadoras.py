...
from ..modelos.modelos import Computadora, Empleado
...
async def crear_computadora(computadora: ComputadoraCrear, db: Session = Depends(obtener_db)):
    # Verificar que el empleado existe si se proporciona id_empleado
    if computadora.id_empleado:
        empleado = db.query(Empleado).filter(Empleado.id == computadora.id_empleado).first()
        if not empleado:
...
async def actualizar_computadora(id_computadora: int, datos: ComputadoraActualizar, db: Session = Depends(obtener_db)):
    ...
    if datos.id_empleado is not None:
        ...
        if not empleado and datos.id_empleado != 0:
            raise HTTPException(status_code=404, detail="Empleado no encontrado")
